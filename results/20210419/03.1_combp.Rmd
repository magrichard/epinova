...
bed$pval = pval
bed$beta = ewas[,"beta"]
bed = bed[,c(1:3,5,6,7,4)]
colnames(bed) = c("chrom", "start", "end", "probes", "pval", "beta", "strand")
...
boxplot(d[idx_probes, idx_sample])
```

```{r echo=T, eval=T}
slk_file = paste0("dmr_", prefix2, ".slk.bed.gz")
if (file.exists(probes_file)) {
  slk = read.table(gzfile(slk_file), comment="@", header=TRUE);
} else {
  warning(paste0("slk_file ", slk_file, " does not exist. Have to fix it! "))
  return(NULL)    
}

slk$beta = bed$beta
slk = slk[order(slk[,1], slk[,2]),]
rownames(slk) = rownames(probes)

# r1 = subset(slk, X.chrom == "chr6" & start >= 33163968 & end <= 33164542)
# r2 = subset(slk, X.chrom == "chr13" & start >= 113158210 & end <= 113158673)
# r3 = subset(slk, X.chrom == "chrX" & start >= 103214961 & end <= 103215296)

#for (i in 1:nrow(new_regions)){
#  subset(slk, X.chrom == new_regions$X.chrom[i] & start >= new_regions$start[i] & end <= new_regions$end[i])
#}

r1 = subset(slk, X.chrom == new_regions$X.chrom[1] & start >= new_regions$start[1] & end <= new_regions$end[1])
r2 = subset(slk, X.chrom == new_regions$X.chrom[3] & start >= new_regions$start[3] & end <= new_regions$end[1])
r3 = subset(slk, X.chrom == new_regions$X.chrom[6] & start >= new_regions$start[6] & end <= new_regions$end[1])

#combp_subset =  apply_func(slk, 1, function(i){
#  set = subset(slk, X.chrom == new_regions$X.chrom[i] & start >= new_regions$start[i] & end <= new_regions$end[i])
#}
#)
```
