...
nb_probes = nrow(d)
bed_ewas_florent = read.table("test/ewas4combp_study_hadaca_meth_raw.rds_model01.bed",comment="@",sep="\t",header=T)
```
...
d = d[rownames(d) %in% rownames(bed_ewas_florent),]
...
if (!file.exists(paste0(prefix,"_DC",".rds"))) {
  ewas_mean_DC =  apply_func(d[1:nb_probes,] , 1, function(l) {  
    # print(l)
    # l = d[1,]
...
if (!file.exists("ewas_var.rds")) {
  ewas_var <- apply_func(data[1:n_probes,] , 1, function(l) {  
    # print(l)
    # l = d[1,]
...
```{r preparing data}
# Mean
colnames(ewas_mean) = c("lpv","beta_C", "beta_D","sd_C","sd_D")
ewas_mean$lpv = -log10(ewas_mean$lpv)
lpval = ewas_mean[,"lpv"]
names(lpval) = rownames(ewas_mean)

#Variance
colnames(ewas_var) = c("lpv","beta")
ewas_var$lpv = -log10(ewas_var$lpv)
lpval_var = ewas_var[,"lpv"]
names(lpval_var) = rownames(ewas_var)
...
bed = pf[rownames(ewas_mean),c(1:3,5)] # Warning, here pf is global, it must be arg and indexed!!
head(bed)
bed$probes = names(pval)
bed$pval = pval
bed$beta_C = ewas_mean[,"beta_C"]
bed$beta_D = ewas_mean[,"beta_D"]
bed$sd_C = ewas_mean[,"sd_C"]
bed$sd_D = ewas_mean[,"sd_D"]
bed = cbind(bed,d[,c(idx_C,idx_D)])
bed = bed[,c(1:3,5:10,4,11:26)]
...
# build and write bed file
bed_var = pf[rownames(ewas_var),c(1:3,5)] # Warning, here pf is global, it must be arg and indexed!!
head(bed_var)
bed_var$probes = names(pval_var)
bed_var$pval = pval_var
bed_var$beta = ewas_var[,"beta"]
bed_var = cbind(bed_var,data[,c(idx_C,idx_D)])
...
head(bed_var)
# write ewas res for combp
bed_ewas_var_filename = paste0("ewas4combp_", prefix_var,"_florent",".bed")
...
```{r combp}
pval_tresh = 10^-3
prefix2 = paste0(prefix, "_", pval_tresh,".fabien")
# # Run comb-p, run!!
cmd = "comb-p"
...
#bed_ewas = read.table("test/ewas4combp_study_hadaca_meth_raw.rds_model01.bed",comment="@",sep="\t",header=T)
...
test_probes = 3
test = apply_func(d[1:test_probes,c(idx_C,idx_D)],1, function(l){
  #l = d[rownames(d)[1],]
  data = e[colnames(d),]
  data$meth = l
...
## Variance outputs
ewas_var = read.table(paste0("ewas4combp_", prefix_var,"_florent", ".bed"), comment="@",header=TRUE) ## variance outputs (V)
...
for (i in 1:nrow(rois_var)){
  ...
  #layout(matrix(1:4,2))
  #par(mfrow=c(2,2))
  #image(d[idx_probes,idx_sample], col=cols, breaks=breaks, xaxt="n", yaxt="n",ylab="Methylation value between \n C (bottom) and D (top)",cex.lab=0.75)
  #segments(-1,0.5,2,0.5,lwd=2)
  #axis(1, (1:nrow(d[idx_probes,idx_sample]) - 1)/(nrow(d[idx_probes,idx_sample]) - 1), rownames(d[idx_probes,idx_sample]), las = 2)
  #plot_combp_res_var(rois_var[i,],ewas,combp_res_probes_var)
  #boxplot(meth_value~cond+probes,concat,las=2,border=c(2,4),ylim=c(0,1))
  ...
  concat$cond = as.factor(concat$cond)
  #errbar(concat$start,concat$mean_value,concat$mean_value+concat$sd_value,concat$mean_value-concat$sd_value,col=c(2,4)[concat$cond],errbar.col=c(2,4)[concat$cond],xlab = "start",ylab = "meth_value")
  concat[concat$chrom=="chrM",1] = "chr23"
  concat[concat$chrom=="chrX",1] = "chr24"
  concat[concat$chrom=="chrY",1] = "chr25"
  mychrs = unique(concat$chrom)
  concat$chrom = as.numeric(factor(concat$chrom),levels=mychrs)
  man = manhattan(ewas_manhattan_var,chr="chrom",bp="start",p="pval",snp = "probes",highlight = idx_probes, cex=sqrt(rois_var[i,]$n_probes),chrlabs = c(1:22,"M","X","Y"),suggestiveline = F)
  print(man)
}
...
cowplot::plot_grid(graphiques, labels = "AUTO")
df$facteur == factor(dfr$condition, levels =  c("n = 10", "n = 15", "n = 20", "n = 30", "n = 40", "n = 50", "n = 100", "n = 500", "α0 = 1", "α0 = 10",  "α0 = 100" , "α0 = 10,000"  , "ε = 0.05", "ε = 0.1",  "ε = 0.2",  "G = 1", "G = 2"))
df$facteur = factor(df$facteur, levels =  c("ordre des facteurs"))
```
...
for (i in 1:nrow(rois)){
  ...
  concat = rbind(cd_C,cd_D)
  #par(mfrow=c(2,3))
  #p1 = as.ggplot(function() plot(as.numeric(as.factor(substr(idx_sample,1, 1))), 1:length(idx_sample),xaxt="n",yaxt="n"))
  #axis(1,c(1,2),levels(e$cond))
  #p2 = as.ggplot(function() image(d[idx_probes,idx_sample], col=cols, breaks=breaks, xaxt="n", yaxt="n",ylab="Methylation value between \n C (bottom) and D (top)"))
  #segments(-1,0.5,2,0.5,lwd=2)
  #axis(1, (1:nrow(d[idx_probes,idx_sample]) - 1)/(nrow(d[idx_probes,idx_sample]) - 1), rownames(d[idx_probes,idx_sample]), las = 2)
  #p3 = as.ggplot(function() plot_combp_res(rois[i,],ewas,combp_res_probes))
  #p4 = as.ggplot(function()boxplot(meth_value~cond+probes,concat,las=2,border=c(2,4),ylim=c(0,1)))
  #p1 = ggplot(concat, aes(x=start, y=mean_value, color=cond)) + geom_point() + geom_errorbar(aes(ymin=mean_value-sd_value, ymax=mean_value+sd_value))
  #print(p1)
  concat$cond = as.factor(concat$cond)
  #p5 = as.ggplot(function()errbar(concat$start,concat$mean_value,concat$mean_value+concat$sd_value,concat$mean_value-concat$sd_value,col=c(2,4)[concat$cond],errbar.col=c(2,4)[concat$cond],xlab = "start",ylab = "meth_value"))
  #cowplot::plot_grid(list(p1,p2,p3,p4,p5),labels="AUTO")
  concat[concat$chrom=="chrM",1] = "chr23"
  concat[concat$chrom=="chrX",1] = "chr24"
  concat[concat$chrom=="chrY",1] = "chr25"
  mychrs = unique(concat$chrom)
  concat$chrom = as.numeric(factor(concat$chrom),levels=mychrs)
  man = manhattan(ewas_manhattan,chr="chrom",bp="start",p="pval",snp = "probes",highlight = idx_probes, cex=rois[i,]$n_probes)
  print(man)
}
